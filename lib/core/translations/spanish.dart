Map<String, String> de_DE = {
  //new need to add
  "Nearby open restaurants": "In der Nähe",
  "Select location": "Stadt wählen",
  "Hot Deals": "Hot Deals",
  "Top rated restaurants": "Gut bewertete Restaurants",
  "Favourite": "Favoriten",
  "Book deal": "Deal buchen",
  "Benefit": "Vorteil",
  "Opening hours": "Öffnungszeiten",
  "Reusable after": "Wiederverwendbar nach",
  "Edit Profile": "Profil bearbeiten",
  "Contact Us": "Kontaktiere uns",
  "About Us": "Über uns",
  "Become a dealer": "Werde Dealer",
  "Manage subscriptions": "Mitgliedschaft",
  "Subscribe now": "Werde Mitglied",
  "Privacy policy": "Datenschutz",

  "add deal": "Deal zufügen",
  "Redeemed": "Eingelöst",
  "Redeemed Deals": "Eingelöste Deals",
  "All deals": "Alle Deals",
  "Manage deals across all businesses":
      "Verwalte Deals über alle Geschäfte hinweg",
  "Pending for edit": "Genehmigung ausstehend",
  "Add new Business": "Neues Geschäft zufügen",
  "Active time frames": "Aktive Zeitfenster",
  "Benefit Amount": "Vorteil",
  "Maximum claims": "Maximal einlösbar",
  "Overall Feedback": "Gesamtfeedback",
  "Select your rating": "Wähle deine Bewertung",
  "Sort By": "Sortieren nach",
  "Dashboard": "Dashboard",
  "0 deals redeemed this month": "0 Deals eingelöst/Monat",

  //already exist.
  //
  //===============
  "Your Businesses": "Deine Unternehmen",

  "Edit": "Bearbeiten",
  "Pause": "Pause",
  "Delete": "Löschen",
  "Active Deals": "Aktive Deals",
  "Total Reviews": "Alle Rezensionen",
  "Active": "Aktiv",
  "Description": "Beschreibung",
  "Deal Type": "Deal",
  "Remove": "Entfernen",
  "Save": "Speichern",
  "Rating Distribution": "Bewertungsverteilung",
  "Filters": "Filter",
  "Business": "Geschäft",
  "Select your business": "Wähle dein Geschäft",
  "Select your sort": "Wähle deine Sortierung",

  "Deals": "Deals",
  "Search restaurants, foods...": "Suche...",
  "See all": "Alle ansehen",
  "Activities": "Aktivitäten",
  "New": "Neu",
  "Home": "Home",
  "Explore": "Entdecke",
  "Profile": "Profil",
  "Price": "Preis",
  "Reviews": "Rezensionen",
  "Information": "Information",
  "Ratings & Reviews": "Bewertungen & Rezensionen",
  "Location": "Location",
  "Contact": "Kontakt",
  "Map": "Map",
  "List": "Liste",
  "Shake your phone to redeem": "Schütteln zum Einlösen",
  "You can redeem from here also": "Oder direkt hier Einlösen",
  "Direction": "Adresse",
  "Share": "Teilen",
  "Your Journey": "Deine Journey",
  "Settings": "Einstellungen",
  "Language": "Sprache",

  'Select Language': 'Sprache auswählen',
  'English': 'Englisch',
  'German': 'Deutsch',

  'Privacy Policy': 'Datenschutz',
  'Terms and Condition': 'Allgemeine Geschäftsbedingungen',
  'Become a Dealer': 'Händler werden',
  'Subscription': 'Abonnement',
  'Log Out': 'Abmelden',
  'Do you want to Log Out?': 'Möchten Sie sich abmelden?',
  'Cancel': 'Abbrechen',

  'Welcome!': 'Willkommen!',
  'Are You Dealer or User?': 'Sind Sie Händler oder Benutzer?',
  'User': 'Benutzer',
  'Dealer': 'Händler',

  'General Settings': 'Allgemeine Einstellungen',
  'Change Password': 'Passwort ändern',
  'Delete Account': 'Konto löschen',
  'Do you want to delete your account?': 'Möchten Sie Ihr Konto löschen?',

  'Email': 'E-Mail',
  'Enter your email': 'Geben Sie Ihre E-Mail ein',
  'Full name': 'Vollständiger Name',
  'Enter your name': 'Geben Sie Ihren Namen ein',
  'Postcode': 'Postleitzahl',
  'Enter your postcode': 'Geben Sie Ihre Postleitzahl ein',

  'If you face any kind of problem with our service feel free to contact us.':
      'Wenn Sie ein Problem mit unserem Service haben, kontaktieren Sie uns bitte.',

  'Current Password': 'Aktuelles Passwort',
  'Enter your password': 'Geben Sie Ihr Passwort ein',
  'New Password': 'Neues Passwort',
  'Confirm New Password': 'Neues Passwort bestätigen',
  'Forgot Password': 'Passwort vergessen',

  'Favorite': 'Favoriten',
  'Details': 'Details',

  'Deutsch': 'Deutsch',

  'Discovering new restaurants, trying delicious dishes while paying less, a review of your culinary journey!':
      'Neue Restaurants entdecken, köstliche Gerichte probieren und dabei sparen – ein Rückblick auf Ihre kulinarische Reise!',

  'Visited Place': 'Besuchte Orte',
  'Cities': 'Städte',
  'Ratings': 'Bewertungen',
  'Review': 'Rezension',

  'Exclusive Deals': 'Exklusive Angebote',
  'Access special promotions and discounts available only to DEALR. users.':
      'Greifen Sie auf spezielle Aktionen und Rabatte zu, die nur für DEALR.-Benutzer verfügbar sind.',
  'Read & Write Reviews': 'Bewertungen lesen & schreiben',
  'Share your experiences and learn from others with ratings and detailed reviews.':
      'Teilen Sie Ihre Erfahrungen und lernen Sie von anderen mit Bewertungen und detaillierten Rezensionen.',
  'Discover Local Gems': 'Lokale Schätze entdecken',
  'Find the best restaurants near you, with personalized recommendations and detailed profiles.':
      'Finden Sie die besten Restaurants in Ihrer Nähe, mit personalisierten Empfehlungen und detaillierten Profilen.',
  'Next': 'Weiter',
  'Start Managing': 'Jetzt starten',
  'Skip': 'Überspringen',

  'Notification': 'Benachrichtigung',
  'just now': 'Gerade eben',
  '{minutes} minutes ago': 'Vor @minutes Minuten',
  '{hours} hours ago': 'Vor @hours Stunden',
  '{days} days ago': 'Vor @days Tagen',
  'There’s no notifications': 'Es gibt keine Benachrichtigungen',
  'Your notifications will appear on this page.':
      'Ihre Benachrichtigungen werden auf dieser Seite angezeigt.',

  'Menu': 'Menü',
  'Currently Closed': 'Derzeit geschlossen',
  'Opens at {time}': 'Öffnet um @time',
  'Opened at {time}': 'Opened at @time',

  'All reviews {number}': 'Alle Bewertungen @number',

  'Nearby Open Restaurants': 'In der Nähe',

  'Hot Deals 🔥': 'Hot Deals 🔥',
  'Top rated Restaurants': 'Gut bewertete Restaurants',

  'Go to deals': 'Zu den Angeboten',
  'Deal booked!': 'Angebot gebucht!',
  'Enjoy the deal at {restaurant}.':
      'Genießen Sie das Angebot im {restaurant}.',

  'Monday': 'Montag',
  'Tuesday': 'Dienstag',
  'Wednesday': 'Mittwoch',
  'Thursday': 'Donnerstag',
  'Friday': 'Freitag',
  'Saturday': 'Samstag',
  'Sunday': 'Sonntag',

  'No favorites yet': 'Noch keine Favoriten',
  'Start exploring our recipes and save your favorites to build your personal collection':
      'Beginnen Sie mit dem Entdecken unserer Rezepte und speichern Sie Ihre Favoriten, um Ihre persönliche Sammlung aufzubauen',
  'Start Exploring': 'Starten Sie die Erkundung',

  'What do you want to do': 'Was möchten Sie tun',
  'Restaurants': 'Restaurants',

  'How was your experience at': 'Wie war Ihre Erfahrung bei',
  'Rating:': 'Bewertung:',
  'Comment:': 'Kommentar:',
  'Let others know about your experience..':
      'Teilen Sie anderen Ihre Erfahrung mit..',
  'Continue': 'Fortfahren',
  'Remind me later': 'Später erinnern',

  "Your Rating": "Ihre Bewertung",
  "Reusable After": "Wiederverwendbar nach",
  "LOCATION": "STANDORT",

  "Open Time :": "Öffnungszeiten :",
  "Price Range :": "Preisspanne :",
  "Available Deals": "Verfügbare Angebote",
  "Location :": "Standort :",
  "Used Deals": "Genutzte Angebote",

  "Congratulations you have used a new deal in your city.":
      "Herzlichen Glückwunsch, du hast ein neues Angebot in deiner Stadt genutzt.",

  "Create Account": "Konto erstellen",
  "Password": "Passwort",
  "Confirm Password": "Passwort bestätigen",
  "Agree with ": "Stimmen Sie zu ",
  "Terms and Conditions": "Allgemeine Geschäftsbedingungen",
  "Sign Up": "Registrieren",
  "Sign Up With Google": "Mit Google registrieren",
  "Already have an account? ": "Sie haben bereits ein Konto? ",
  "Sign In": "Anmelden",

  "Get Verification Code": "Verifizierungscode erhalten",
  "Forget Your ": "Passwort vergessen ",
  "Enter your email address to reset your password.":
      "Geben Sie Ihre E-Mail-Adresse ein, um Ihr Passwort zurückzusetzen.",

  "Verify Email": "E-Mail verifizieren",
  "Enter ": "Geben Sie ",
  "Verification ": "Verifizierung ",
  " Code.": " Code ein.",
  "Enter the code that was sent to your email.":
      "Geben Sie den Code ein, der an Ihre E-Mail gesendet wurde.",
  "Didn't receive the code? ": "Code nicht erhalten? ",
  "Resend": "Erneut senden",
  "Verify": "Bestätigen",

  "Reset Password": "Passwort zurücksetzen",
  "Now Reset Your ": "Setzen Sie jetzt Ihr ",
  "Password.": "Passwort zurück.",
  "Password  must have 6-16 characters.":
      "Das Passwort muss 6-16 Zeichen haben.",

  "Sign In to Your Account": "Melden Sie sich bei Ihrem Konto an",
  "Remember me": "Angemeldet bleiben",
  "Sign In With Google": "Mit Google anmelden",
  "Don't have an account? ": "Noch kein Konto? ",

  "Now continue after register in \"USER\".":
      "Jetzt nach der Registrierung als „USER“ fortfahren.",

  "DeshBoard": "Instrumententafel",
  "Bussiness": "Geschäft",

  "Edit Deal": "Angebot bearbeiten",
  "Add a new deal to attract more customers to your restaurant.":
      "Fügen Sie ein neues Angebot hinzu, um mehr Kunden in Ihr Restaurant zu locken.",
  "Business*": "Geschäft*",

  "Deal Title*": "Angebotstitel*",
  "e.g., Happy Hour Special": "z.B. Happy Hour Special",

  "Describe your deal...": "Beschreiben Sie Ihr Angebot...",
  "Active Time Frames*": "Aktive Zeitfenster*",
  "Add": "Hinzufügen",

  "Day": "Tag",
  "Select day": "Wählen Sie den Tag",
  "Start Time": "Startzeit",
  "End Time": "Endzeit",
  "* Time frames must be at least 4 hours long":
      "* Zeitfenster müssen mindestens 4 Stunden dauern",
  "Deal Type*": "Angebotsart*",
  "Reusable After*": "Wiederverwendbar nach*",
  "60 Days": "60 Tage",
  "Maximum Claims*": "Maximal einlösbar*",
  "e.g. 100": "z.B. 100",

  "Hi Dealer!": "Hallo Händler!",
  "Quick Add Deal": "Schnellangebot hinzufügen",

  "Avg. Rating": "Durchschnittsbewertung",
  "Businesses": "Restaurants",

  "Monthly Performance": "Monatliche Leistung",
  "Total monthly deals redeems across all businesses.":
      "Gesamte monatliche Einlösungen von Angeboten über alle Unternehmen hinweg.",
  "Your Restaurants": "Ihre Restaurants",
  "_ deals redeemed this month": "@number Deals eingelöst/Monat",

  "REDEENED": "EINGELÖST",

  "Why do you want to pause this deal?":
      "Warum möchten Sie dieses Angebot pausieren?",
  "Enter your reason...": "Geben Sie Ihren Grund ein...",
  "Submit": "Absenden",

  "Add Deal": "Deal zufügen",
  "+ Add Deal": "+ Deal zufügen",
  "Benefit Amount*": "Vorteil*",

  "Create Deal": "Deal erstellen",

  "Average Rating": "Durchschnittliche Bewertung",

  "Rating": "Bewertung",

  "Used :": "Verwendet :",

  "Select your ratting": "Wählen Sie Ihre Bewertung",

  "Back to User": "Zurück zum Benutzer",

  "Register": "Registrieren",
  "Business Name": "Firmenname",
  "Search your business": "Suchen Sie Ihr Unternehmen",
  "Your Name": "Ihr Name",
  "Enter your full name": "Geben Sie Ihren vollständigen Namen ein",
  "Your Email": "Ihre E-Mail",
  "Your Phone Number": "Ihre Telefonnummer",
  "Enter your phone": "Geben Sie Ihre Telefonnummer ein",
  "How did you hear about us?": "Wie haben Sie von uns erfahren?",
  "Referral source": "Empfehlungsquelle",
  "Your request is under review. You will get a notification after acceptance.":
      "Ihre Anfrage wird überprüft. Sie erhalten nach der Annahme eine Benachrichtigung.",
  "Go to Customer": "Zum Kunden",

  "Manage Your Offers": "Verwalten Sie Ihre Angebote",
  "Create and manage deals, update your menu, and showcase your restaurant to attract more customers.":
      "Erstellen und verwalten Sie Angebote, aktualisieren Sie Ihre Speisekarte und präsentieren Sie Ihr Restaurant, um mehr Kunden anzuziehen.",

  "Track Your Performance": "Verfolgen Sie Ihre Leistung",
  "Monitor deal redemptions, customer engagement, and revenue insights with comprehensive analytics.":
      "Überwachen Sie Angebotseinlösungen, Kundenengagement und Umsatzeinblicke mit umfassender Analytik.",

  "Grow Your Customer Base": "Erweitern Sie Ihre Kundenbasis",
  "Reach new customers, build loyalty with existing ones, and increase repeat visits through targeted promotions.":
      "Erreichen Sie neue Kunden, stärken Sie die Treue bestehender und erhöhen Sie wiederholte Besuche durch gezielte Aktionen.",

  "Edit Price": "Preis bearbeiten",
  "Item Name": "Artikelname",
  "Enter name": "Name eingeben",
  "Item Description": "Artikelbeschreibung",
  "Enter a short description": "Kurze Beschreibung eingeben",

  "Enter price": "Preis eingeben",
  "+ Add More Item": "+ Weitere Artikel hinzufügen",
  "Save Price": "Preis speichern",

  "Edit Business": "Geschäft bearbeiten",
  "Basic Information": "Basisinformationen",
  "Business Name*": "Geschäftsname*",
  "Enter your restaurant name": "Geben Sie den Namen Ihres Restaurants ein",
  "Categories*": "Kategorien*",
  "Select a category": "Kategorie auswählen",
  "Business Image": "Geschäftsbild",
  "Upload your image": "Bild hochladen",

  "ZIP/Postal Code*": "PLZ*",
  "e.g., Downtown, Mall District": "z.B., Innenstadt, Einkaufszentrum",
  "Detailed Address*": "Detaillierte Adresse*",
  "e.g., 123 Main Street, Building Name": "z.B., Hauptstraße 123, Gebäudename",
  "Phone Number*": "Telefonnummer*",
  "+880 126 2548 255": "+880 126 2548 255",
  "Opening Hours": "Öffnungszeiten",
  "Closed": "Geschlossen",
  "to": "bis",
  "Save Business": "Geschäft speichern",

  "Add New Business": "Neues Geschäft zufügen",
  "Chef's Table": "Chef's Tisch",
  "Delete Item": "Artikel löschen",
  "Are you sure you want to delete this item? This action cannot be undone.":
      "Möchten Sie diesen Artikel wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.",
  "Downtown, 123 Main St": "Innenstadt, Hauptstraße 123",
  "deals": "Angebote",
  "deals redeemed this month": "Angebote diesen Monat eingelöst",

  "Add Price": "Preis hinzufügen",

  "Add Business": "Geschäft hinzufügen",
  "Manage deals across all your businesses":
      "Verwalten Sie Angebote in all Ihren Unternehmen.",
  "Become a User": "Benutzer werden",

  "Avg Rating": "Durchschnittliche Bewertung",
  "Monthly performance": "Monatliche Performance",
  "Total monthly deals redeems across all businesses":
      "Gesamtzahl der monatlich eingelösten Angebote in allen Restaurants.",

  "Deal redeemed!": "Deal eingelöst!",
  "This deal reservation does not contain a general reservation. You can just come to the location and redeem the deal. For a reservation, please contact the DEALR.":
      "Diese Deal-Reservierung enthält keine allgemeine Reservierung. Du kannst einfach zur Location kommen und den Deal einlösen. Für eine Reservierung wende dich bitte an den jeweilegen DEALR.",
  "The deal is only valid for dine-in and does not apply to takeaway.":
      "Nur gültig beim Essen im Lokal, nicht zum Mitnehmen.",
};
